...
public:
    ...
    const std::vector<TargetTrace> &getTraces() const { return traces; }
    size_t getTraceCount() const { return traces.size(); }
    ...
    void addTrace(const TargetTrace &trace) { traces.push_back(trace); }

    // 获取雷达模式
    int getRadarMode() { return modeType; }

private:
    std::vector<TargetInfo> targets;
    std::vector<TargetTrace> traces; // 协议0的数据
    size_t targetCount = 0;
    int modeType = 0; // 安防：0，点云通用版：2
};
...
using CommandParseResult = std::variant<
    ...
    std::vector<TargetInfoParse_0xA8::TargetTrace>>;
...
public:
    ...
    static bool parseFrame_0(const uint8_t * data, size_t len, ProtocolFrame &frame);
...
private:
    ...
    // static constexpr size_t RECV_BUFFER_SIZE = 40000;   // 防止延迟过大，及时清除过期数据
    ...
    int m_modeType = 0;     // 设置为安防点迹模式
    std::chrono::high_resolution_clock::time_point frameStartTime;  // 新增

public:
    explicit TcpCommandHandler(const std::string &address = "192.168.10.117", int port = 50000)
        : tcpClient(std::make_shared<TcpClient>(address, port)), parseState(ParseFrameState::START), parseBufCount(0), expectedDataLen(0)
    {
    ...
    int getModeType() const{return m_modeType;}
