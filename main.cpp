...
void testRadarFileReader()
{
    ...
    // 读取并打印数据
    int frameCount = 0;
    while (!reader->isEnd() && frameCount < 100)
    { // 读取10帧数据
        if (reader->readNext())
        {
...
void testDBScan()
{
    cv::FileStorage fs1("camera_calibration.yml", cv::FileStorage::READ);
    cv::Mat camera_matrix, dist_coeffs;
    fs1["camera_matrix"] >> camera_matrix;
    fs1["dist_coeffs"] >> dist_coeffs;
    fs1.release();

    cv::FileStorage fs2("E:/Source/4DRadarTest/extrinsic_calibration.yml", cv::FileStorage::READ);
    cv::Mat rvec, tvec;
    // 从 YAML 文件中读取
    fs2["rotation_vector"] >> rvec;
    fs2["translation_vector"] >> tvec;
    fs2.release();

    // 从文件中读取数据
    auto radar_reader = DataReaderFactory::createReader(ReaderType::RADAR_FILE, "E:/dataset/manaulCalib/radar");
    if (!radar_reader->init())
    {
        std::cerr << "初始化读取器失败" << std::endl;
        return;
    ...
    auto image_reader = DataReaderFactory::createReader(ReaderType::IMAGE_FILE, "E:/dataset/manaulCalib//camera_near");
    if (!image_reader->init())
    {
        std::cerr << "初始化读取器失败" << std::endl;
        return;
...
void testExtrinsicManualCalib(){
    CalibRadarVisualizer visualizer(
        "camera_calibration.yml",
        "new_extrinsic_calibration.yml",
        // "E:/Source/4DRadarTest/build/Debug/sync_data_20250114_163251/radar",    // radar_path
...
void testDetectModel(){
    YOLOv6ONNX model(L"E:/Source/4DRadarTest/models/last_ckpt.onnx");

    // 加载图像读取器
    std::string image_path = "E:/dataset/manualCalib_h/camera_near";
    std::string radar_path = "E:/dataset/manualCalib_h/radar";
    auto radar_reader_ = std::dynamic_pointer_cast<RadarFileReader>(DataReaderFactory::createReader(ReaderType::RADAR_FILE, radar_path));
    auto image_reader_ = std::dynamic_pointer_cast<ImageFileReader>(DataReaderFactory::createReader(ReaderType::IMAGE_FILE, image_path));
    if (!radar_reader_->init() || !image_reader_->init())
    {
        std::cout << ("Failed to initialize readers");
        return;
    }
    int count = 6;
    ...
    cv::Mat image = std::dynamic_pointer_cast<ImageData>(image_reader_->getData())->frame.clone();
    if(image.empty()) {
    ...
    // 映射实验
    class RadarImageMapper mapper;
    mapper.init("camera_calibration.yml", "E:/Source/4DRadarTest/extrinsic_calibration.yml");
    auto radar_data = std::dynamic_pointer_cast<RadarData>(radar_reader_->getData())->points;
    cv::Mat depthMask = mapper.createDepthMask(radar_data, image.size());
    // // 快速归一化显示（0-300米 -> 0-255灰度）
    // cv::Mat depth_vis;
    // depthMask.convertTo(depth_vis, CV_8UC1, 255.0/300.0); // 300米对应255
    // // 将无效区域（0值）设为黑色
    // cv::threshold(depth_vis, depth_vis, 1, 255, cv::THRESH_BINARY);
    // cv::cvtColor(depth_vis, depth_vis, cv::COLOR_GRAY2BGR);
    // cv::imshow("Depth Mask", depth_vis);
    ...
    for (const auto &point : radar_data)
    {
    ...
    for (const auto& res : results) {
        ...
        cv::putText(image, label, res.box.tl(), 
                   cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 255, 0), 1);
    }
    mapper.mapperProject(radar_data, image);
